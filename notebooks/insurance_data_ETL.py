...
#  by territory, while town was not. Reconstructed dimension table to enforce 1-row-per-territory grain to ensure correct
...
# Minimum volume thresshold on model_pricing_ratio for territories where the policy count is greater than 50
...
# Create a stability flag for the current premium column to ingore extreme outliers
premiums["min_premium_flag"] = premiums["current_premium"] >= 25

#TODO Create a stability flag  
premiums["stable_pricing_flag"] = (
    premiums["stable_indicated_ratio_flag"] &
    premiums["min_premium_flag"]
)

#TODO When creating the dim_r table, validated referential integrity and restricted the dimension to active keys in the fact table to ensure clean joins and prevent orphan reference data.
